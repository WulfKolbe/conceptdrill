...
EVERY FIELD MUST FIT {ceiling} TOKENS OF A SENTENCE EMBEDDING MODEL. That is
about {ceiling_words} words. A longer text is not richer: it is averaged over more tokens, and the
...
summary      {summary} words of prose. The concept as this document presents it,
             in the author's own terminology, preserving its scope and its
             qualifications. The most specific of the three.

abstraction  {abstraction} words of prose. The same idea stated so it makes sense
...
label        {label} words forming A NOUN PHRASE. Not a sentence. This is the
...
- {label} words. Count silently. A {label_low_fail}-word label is a failure;
  so is a {label_high_fail}-word one. Add discriminating detail rather than padding.
...
{"concepts": [
  {"summary": "<{summary} words, prose, this document's terminology>",
   "abstraction": "<{abstraction} words, prose, document-independent, no local proper nouns>",
   "label": "<{label} words, a noun phrase, no copula, no acronyms, no citations>"}
]}
